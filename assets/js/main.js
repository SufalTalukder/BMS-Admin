...
function formatDate(dateString) {
  ...
  // Extract time components and format to 12-hour time
  let [hour, minute, second] = timePart.split(':');
  hour = parseInt(hour, 10);
  const isPM = hour >= 12;
  const displayHour = hour % 12 || 12; // Converts to 12-hour format

  // Format date and time
  const formattedDate = `${month}/${day}/${year}`;
  const formattedTime = `${displayHour}:${minute} ${isPM ? 'PM' : 'AM'}`;
